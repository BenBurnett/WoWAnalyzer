...
class SoulReaper extends ExecuteHelper {
  ...
  private darkTransformationActive = false;
  private dtWindowOpen = false;
  private wastedDTWindows = 0;
  ...
  private readonly darkTransformationStartByWindowId = new Map<number, number>();
  private putrefyChargesAvailable = 0;
  ...
  constructor(options: Options) {
    ...
    if (this.selectedCombatant.hasTalent(TALENTS.PUTREFY_TALENT)) {
      this.putrefyChargesAvailable = this.selectedCombatant.hasTalent(TALENTS.PUTRID_ECHOES_TALENT)
        ? 2
        : 1;
    }
    ...
    // one free cast on any target; the SpellUsable subclass handles the CD reset.
    this.addEventListener(
  ...
  private onSRCast(event: CastEvent) {
    this.soulReaperCasts.push({
      ...
      duringDarkTransformation: this.darkTransformationActive,
      darkTransformationWindowId: this.currentDarkTransformationWindowId,
      putrefyChargesAtCast: this.putrefyChargesAvailable,
    });
  ...
  private onPutrefyCooldownUpdate(event: UpdateSpellUsableEvent) {
    if (
      ...
      this.putrefyChargesAvailable = event.chargesAvailable;
    }
  }
  ...
  private closeDarkTransformationWindow(fallbackTimestamp: number) {
    ...
    this.missedFreeSoulReaperWindows.push({
      ...
      timestamp:
        this.darkTransformationStartByWindowId.get(darkTransformationWindowId) ?? fallbackTimestamp,
    });
    this.wastedDTWindows += 1;
    this.dtWindowOpen = false;
  }
  ...
  private getFreeDarkTransformationCastsDuringExecute(): number {
    ...
    for (const cast of this.soulReaperCasts) {
      if (!cast.duringDarkTransformation || cast.darkTransformationWindowId === null) {
        continue;
      }
  ...
  ): { performance: QualitativePerformance; assessment: JSX.Element } {
  ...
  get guideSubsection(): JSX.Element {
    ...
    const data = (
      <CastDetail
        ...
        filters={[QualitativePerformance.Good, QualitativePerformance.Fail]}
      />
    );
      ...
      >
        <BoringSpellValueText spell={TALENTS.SOUL_REAPER_TALENT}>
          ...
          {this.wastedDTWindows > 0 && (
            <div>
              <span style={{ color: 'red' }}>{this.wastedDTWindows}</span>{' '}
              <small>wasted Dark Transformation window{this.wastedDTWindows > 1 ? 's' : ''}</small>
            </div>
          )}
